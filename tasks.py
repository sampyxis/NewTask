...
class InboxPage(BaseRequestHandler):
  ...
  @login_required
  def get(self):
    lists = model.TaskList.get_current_user_lists()
...
class TaskListPage(BaseRequestHandler):
  ...
  def get(self):
    task_list = model.TaskList.get(self.request.get('id'))
    if not task_list:
      self.error(403)
      return
    ...
    if not task_list.current_user_has_access():
      if task_list.published:
        if output_name == 'default':
          output_name = 'html'
          output_type = model.TaskListPage._OUTPUT_TYPES[output_name]
      else:
        user = users.get_current_user()
        if not user:
...
class CreateTaskListAction(BaseRequestHandler):
  ...
  def post(self):
    ...
    task_list = model.TaskList(name=name)
    task_list.put()
...
class EditTaskAction(BaseRequestHandler):
  ...
  def post(self):
    ...
    if task_key:
      ...
      task = None
      task_list = model.TaskList.get(self.request.get('list'))
    ...
    if task:
      ...
      task = model.Task(description=db.Text(description), task_list=task_list)
    task.put()
...
class AddMemberAction(BaseRequestHandler):
  ...
  def post(self):
    ...
    if not task_list.user_has_access(user):
      member = model.TaskListMember(user=user, task_list=task_list)
      member.put()
    self.redirect(self.request.get('next'))
...
class SetTaskCompletedAction(BaseRequestHandler):
  ...
  def post(self):
    task = model.Task.get(self.request.get('id'))
    if not task or not task.task_list.current_user_has_access():
      self.error(403)
      return
